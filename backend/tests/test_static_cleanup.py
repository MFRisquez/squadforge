...
def test_transfer_rail_hidden_on_phone_page_fit():
    ...
    assert 'CACHE = "futfantasy-v180"' in sw
    ...
    assert "/static/styles.css?v=180" in sw
    assert "/static/lineup.js?v=180" in sw
    assert "/static/appshell.js?v=180" in sw
    assert "/static/fixtures.js?v=180" in sw
    assert "/static/league_h2h.js?v=180" in sw
    assert "/static/club-sheet.js?v=180" in sw
    ...
    assert "sw.js?v=180" in base
    assert "styles.css?v=180" in base
    assert "appshell.js?v=180" in base
    ...
    assert "loadScriptText" in appshell
    ...
    assert "lineup.js?v=180" in lineup_tpl
    assert "club-sheet.js?v=180" in lineup_tpl
    fx_tpl = (TEMPLATES / "fixtures.html").read_text(encoding="utf-8")
    assert "fixtures.js?v=180" in fx_tpl
    css = (STATIC / "styles.css").read_text(encoding="utf-8")
