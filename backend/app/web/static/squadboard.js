(async () => {
  ...
  function setTdValue(code) {
    const sel = tdSelect();
    if (!sel) return;
    sel.value = code || "";
    const btn = document.getElementById("tdPickBtn");
    if (btn) btn.textContent = code ? "Change DT" : "Pick club";
  }
  ...
  function pickFromTransferRail(p) {
    ...
    if (freeEdit()) {
      ...
      if (!target) {
        ...
        if (idx < 0) {
          alert(`No empty ${p.position} slot — remove someone first.`);
          return;
        }
        target = { pos: p.position, index: idx };
      }
      if (p.position !== target.pos) {
        alert(`That slot needs a ${target.pos}.`);
        return;
      }
      active = target;
      pickerMode = "add";
    ...
    alert("Transfer someone out on the pitch first.");
  }
  ...
  function renderPicker() {
    ...
    const excludeForBudget =
      pickerMode === "transfer" && outPlayer ? outPlayer.id : currentId || null;
    ...
    const baseSpend = filledIds().reduce((s, id) => {
      if (excludeForBudget && id === excludeForBudget) return s;
      return s + (byId[id]?.price || 0);
    }, 0);
    const maxAfford = BUDGET - baseSpend;
  ...
  function paintTdCorner(info) {
    ...
    if (!meta) {
      ...
      const anchor = corner.querySelector("#tdPickBtn") || corner.querySelector("#tdClub");
      if (anchor) corner.insertBefore(meta, anchor);
  ...
  async function saveTdClub(select) {
    const club = (select && select.value) || currentTd();
    if (!club) throw new Error("Pick a DT club first");
    const corner = document.querySelector(".td-corner");
    const pickBtn = document.getElementById("tdPickBtn");
  ...
  /* —— DT club picker / detail sheet —— */
  let tdBoot = { banned: null, can_set: false };
  try {
    const bootEl = document.getElementById("tdBoot");
    if (bootEl) tdBoot = JSON.parse(bootEl.textContent || "{}") || tdBoot;
  } catch (_) {}

  const clubPicker = document.getElementById("clubPicker");
  const clubPickerList = document.getElementById("clubPickerList");

  function closeClubDrawer(el) {
    if (typeof window.__ffCloseClubDrawer === "function") {
      return window.__ffCloseClubDrawer(el);
    }
  ...
  function openClubPicker() {
    if (!clubPicker || !clubPickerList || LOCKED) return;
    const banned = tdBoot.banned || "";
    const qs = banned ? `?exclude=${encodeURIComponent(banned)}` : "";
    clubPickerList.innerHTML = `<li class="muted tiny" style="padding:0.75rem">Loading clubs…</li>`;
    ...
    fetch(`/api/clubs${qs}`)
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((data) => {
        const clubs = (data.clubs || []).filter((c) => !c.banned);
        if (!clubs.length) {
          clubPickerList.innerHTML = `<li class="muted tiny" style="padding:0.75rem">No clubs available.</li>`;
          return;
        }
        clubPickerList.innerHTML = "";
        clubs.forEach((c) => {
          const li = document.createElement("li");
          li.innerHTML = `
            <button type="button" class="club-pick-row" data-club="${c.code}">
              ${c.badge ? `<img class="club-pick-badge" src="${c.badge}" alt="" width="36" height="36" loading="lazy" />` : `<span class="club-pick-badge is-empty"></span>`}
              <span class="club-pick-copy">
                <strong>${c.name}</strong>
                <span class="muted">${c.code}</span>
              </span>
            </button>`;
          li.querySelector("button").addEventListener("click", () => openClubDetail(c.code));
          clubPickerList.appendChild(li);
        });
      })
      .catch(() => {
        clubPickerList.innerHTML = `<li class="muted tiny" style="padding:0.75rem">Couldn’t load clubs.</li>`;
      });
  }

  const tdForm = document.getElementById("tdForm");
  const tdPickBtn = document.getElementById("tdPickBtn");
  if (tdForm) {
    tdForm.addEventListener("submit", (e) => e.preventDefault());
  }
  if (tdPickBtn) {
    tdPickBtn.addEventListener("click", () => openClubPicker());
  }
  const closeClubPickerBtn = document.getElementById("closeClubPicker");
  if (closeClubPickerBtn) {
    closeClubPickerBtn.addEventListener("click", () => closeClubDrawer(clubPicker));
  }
  if (clubPicker) {
    clubPicker.addEventListener("click", (e) => {
      if (e.target === clubPicker) closeClubDrawer(clubPicker);
    });
  }
  if (typeof window.__ffBindClubDetail === "function") {
    window.__ffBindClubDetail();
  }
  /* Transfers: tap current DT corner to view club sheet */
  const tdCorner = document.querySelector(".td-corner:not(.td-corner-xi)");
  if (tdCorner) {
    tdCorner.classList.add("is-club-tappable");
    tdCorner.addEventListener("click", (e) => {
      if (e.target.closest("#tdPickBtn, a, button")) return;
      const code = (document.getElementById("tdClub") || {}).value || "";
      if (code && typeof window.__ffOpenClubDetail === "function") {
