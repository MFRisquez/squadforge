(async () => {
  ...
  function openClubDetail(code) {
    ...
    fetch(`/api/clubs/${encodeURIComponent(code)}`)
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((data) => {
        if (clubDetailCode !== code) return;
        if (data.error) throw new Error(data.error);
        paintClubDetail(data);
      })
      .catch(() => {
        if (clubDetailCode !== code) return;
        if (clubDetailBody) {
  ...
  function paintClubDetail(data) {
    ...
    if (clubDetailActions) {
      ...
      choose.addEventListener("click", () => {
        ...
        saveVisual = "dirty";
        paintSaveBtn();
        closeClubDrawer(clubDetail).then(() => closeClubDrawer(clubPicker));
      });
  ...
  if (closeClubDetailBtn) {
    closeClubDetailBtn.addEventListener("click", () => closeClubDrawer(clubDetail));
  }
  if (clubPicker) {
  ...
  if (clubDetail) {
    clubDetail.addEventListener("click", (e) => {
      if (e.target === clubDetail) closeClubDrawer(clubDetail);
    });
  }
